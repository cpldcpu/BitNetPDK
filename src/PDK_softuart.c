...
// calling overhead is 5 cycles
// sendloop overhead is 5 cycles
// = total of 10 cycle overhead
uartdelay:
    mov	    a,#(((F_CPU/4)/BAUDRATE)-2)
0$:
    sub     a,#1  // 4 cycles per loop
    t1sn	f,z
    goto	0$
    //	ret   
__endasm;
...
//  print_hex8
//  
//  Accepts an uint9 value as parameter and outputs its 
//  hexadecimal representation on the UART using "putchar".
// -------------------------------------------------------

void PDK_sendhex8(uint8_t in) {
    uint8_t nib;
    nib=in>>4;
    PDK_sendchar(nib>9?nib+'A'-10:nib+'0');    
